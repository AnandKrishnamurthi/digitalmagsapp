...
class page:
    def switch(self):
        ...
        for i in range(
        ...
        ):  # for loop runs for the length of the list
            # of all of the subject files
            self.listforbuttons.append(
                ttk.Button(
                    self.frame1,
                    style="big.TButton",
            ...
            self.listforbuttons[i].grid(
                row=y, column=x, pady=20, padx=10
            )  # padding for spaces to make it look cleaner
            x += 1
            if (
                x == 2
            ):  # when x goes above 1,
                # reset to place the third item in the first column
                x = 0
                y += 1  # goes down by 1 when x goes above 1
        self.addsubj_button = ttk.Button(
            self.frame1,
            style="big.TButton",
            text="Add Subject",
            command=lambda: self.change_to_page(3),
        )
