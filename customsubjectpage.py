...
class page:
    def switch(self):
        self.number_of_times_del_clicked = (
            0  # when page created, number of times delete clicked is set to 0
        )
        self.backtosubjects = ttk.Button(
            self, text="Back", command=lambda: self.change_to_page(2)
        )
        self.backtosubjects.place(x=20, y=20, anchor="nw")

        self.titlelbl = ttk.Label(
            self, text=self.current_subject.capitalize(),
            font=("Arial", 40, "bold")
        )
        ...
        self.text_box.insert(
            "end", self.file_contents
        )  # insert all saved data from the relevant text file,
        # into the text box
        ...
        self.updatebtn = ttk.Button(
            ...
            style="big.TButton",
            text="Enter",
            command=lambda: self.update_subject_list(
                self.current_subject, self.text_box.get("1.0", "end")
            ),
        )
